...
import random
import math
from agent import Command, MoveCommand, ShootCommand, SnapshotData
...
def my_ai(gamestate: SnapshotData, my_data: typing.Dict) -> Command:
    ...
    def inDanger(projectile):
        if healthHim > 0 and healthMe > 0:
            ...
            if xSpeed != 0:
                angleTir = (-math.copysign(1, xSpeed)*90+90) + \
                    arctan(ySpeed/xSpeed)*180/3.14
            if (xPro-xMe) != 0:
                angleMe = (-math.copysign(1, xPro-xMe)*90+90) + \
                    arctan((yMe-yPro)/(xPro-xMe+0.001))*180/3.14
            if xSpeed == 0:
